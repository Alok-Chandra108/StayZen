...
module.exports.login = async(req, res, next) => {
    ...
    if (req.user.isLockedOut && req.user.lockUntil > Date.now()) {
        const remainingTime = Math.ceil((req.user.lockUntil - Date.now()) / 60000);
        req.flash("failure", `Account locked due to too many failed attempts. Try again in ${remainingTime} minute${remainingTime > 1 ? 's' : ''}.`);
        return res.redirect("/login");
    }
    ...
    if (req.user.failedLoginAttempts > 0 || req.user.lockUntil) {
